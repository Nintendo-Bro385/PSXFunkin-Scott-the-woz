...
void Menu_Load(MenuPage page)
{
	...
	menu.StoryScott = Char_ScottM_New(FIXED_DEC(-80,1), FIXED_DEC(100,1));
...
void Menu_Unload(void)
{
	...
	Character_Free(menu.StoryScott);
	...
	//Fallthrough
		...
		case MenuPage_Story:
		{
			static const struct
			{
				...
				{"1", StageId_1_1, "BORDERLINE FOREVER", {"STUPID NINTENDO GAMES", "WHY DONT YOU PLAY THEM", "BREAKOUT", "CLOSING IN"}},
			};
			...
			menu.StoryScott->tick(menu.StoryScott);
			
			menu.StoryGF->tick(menu.StoryGF);
			
			//Draw upper strip
			RECT name_bar = {0, 32, SCREEN_WIDTH, 112};
			Gfx_DrawRect(&name_bar, 100, 149, 237);
		...
		case MenuPage_Freeplay:
		{
			static const struct
			{
				...
				{StageId_1_1, 0xFF9271FD, "STUPID NINTENDO GAMES"},
				{StageId_1_2, 0xFF9271FD, "WHY DONT YOU PLAY THEM"},
				{StageId_1_3, 0xFF9271FD, "BREAKOUT"},
				{StageId_2_1, 0xFF9271FD, "CLOSING IN"},
				{StageId_2_2, 0xFF9271FD, "BIBLE"},
			};
